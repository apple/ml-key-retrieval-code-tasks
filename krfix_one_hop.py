...
class TaskBuilder:
    ...
    def three_step(self) -> tuple[list[str], str, str]:
        ...
        call_graph_comment_position = self.cfg.call_graph_comment_position

        def add_comment(comment, func_decl):
            if call_graph_comment_position == "before":
                return comment + func_decl
            elif call_graph_comment_position == "after":
                return func_decl + "\n" + comment
            else:
                raise ValueError(
                    f"Unsupport call_graph_comment_position={call_graph_comment_position}"
                )

        if "called_by_one_hop" in self.cfg.call_graph_comment_type:
            called_by_comment = self.call_graph_comment(
                direction="called_by", func_names=[func_val_2]
            )

            func_1_decl = add_comment(called_by_comment, func_1_decl)

            called_by_comment = self.call_graph_comment(
                direction="called_by", func_names=[func_key]
            )
            func_2_decl = add_comment(called_by_comment, func_2_decl)

        if "calls_one_hop" in self.cfg.call_graph_comment_type:
            calls_comment = self.call_graph_comment(
                direction="calls", func_names=[func_val_2]
            )
            func_key_decl = add_comment(calls_comment, func_key_decl)

            calls_comment = self.call_graph_comment(
                direction="calls", func_names=[func_val_1]
            )
            func_2_decl = add_comment(calls_comment, func_2_decl)
